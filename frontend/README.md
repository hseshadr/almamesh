...
1. **Sync tier** — the injected-worker `EngineClient` from the standalone
   `@edgeproc/browser` package pulls the **signed,
...
> `@edgeproc/browser` repository at the exact commit pinned in its manifest and
