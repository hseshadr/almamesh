...
const migrationWorkflowPath = resolve(root, ".github/workflows/production-secret-migration.yml")
...
function exactMigrationWorkflowViolations(source: string): string[] {
  const workflow = mapping(Bun.YAML.parse(source))
  const jobs = mapping(workflow.jobs)
  const job = mapping(jobs.migrate)
  const violations: string[] = []

  if (workflow.name !== "Production secret migration") violations.push("workflow-name")
  if (!sameValue(mapping(workflow.on), { workflow_dispatch: null })) violations.push("triggers")
  if (!sameValue(workflow.permissions, { contents: "read" })) violations.push("permissions")
  if (!sameValue(Object.keys(jobs), ["migrate"])) violations.push("jobs")
  if (!sameValue(job, migrationJob)) violations.push("migration-job")
  return violations
}
...
const migrationJob = {
  name: "Production secret migration",
  ...
  steps: [
    ...
    {
      name: "Relay the exact production secret allowlist",
      uses: daggerAction,
      env: {
      ...
      with: {
        version: "0.21.8",
        call: "relay-production-secrets --admin-token=env:SECRET_RELAY_ADMIN_TOKEN --bundle-private-key-b-64=env:BUNDLE_PRIVATE_KEY_B64 --bundle-public-key-b-64=env:BUNDLE_PUBLIC_KEY_B64 --cloudflare-account-id=env:CLOUDFLARE_ACCOUNT_ID --cloudflare-api-token=env:CLOUDFLARE_API_TOKEN --operation-id=${{ github.run_id }}-${{ github.run_attempt }}",
      },
    },
  ],
}

const canonicalMigrationFixture = [
  "name: Production secret migration",
  "",
  "on:",
  "  workflow_dispatch:",
...
describe("temporary production secret migration workflow", () => {
  test("is manual, main-only, production-scoped, and Dagger-only", () => {
    expect(exactMigrationWorkflowViolations(canonicalMigrationFixture)).toEqual([])
    const source = readFileSync(migrationWorkflowPath, "utf8")
    expect(exactMigrationWorkflowViolations(source)).toEqual([])
  })

  test.each([
    {
      name: "push trigger",
      source: canonicalMigrationFixture.replace("  workflow_dispatch:\n", "  push:\n"),
      violation: "triggers",
    },
    {
      name: "non-main authorization",
      source: canonicalMigrationFixture.replace(
        "    if: github.ref == 'refs/heads/main'",
        "    if: always()",
      ),
      violation: "migration-job",
    },
    {
      name: "missing production environment",
      source: canonicalMigrationFixture.replace("    environment: production\n", ""),
      violation: "migration-job",
    },
    {
      name: "canceling concurrency",
      source: canonicalMigrationFixture.replace("      cancel-in-progress: false", "      cancel-in-progress: true"),
      violation: "migration-job",
    },
    {
      name: "run-id-only cache buster",
      source: canonicalMigrationFixture.replace(
        "--operation-id=${{ github.run_id }}-${{ github.run_attempt }}",
    ...
    {
      name: "unsplit numeric acronym flags",
      source: canonicalMigrationFixture
        .replace("--bundle-private-key-b-64", "--bundle-private-key-b64")
        .replace("--bundle-public-key-b-64", "--bundle-public-key-b64"),
      violation: "migration-job",
    },
    {
      name: "secret-bearing checkout",
      source: canonicalMigrationFixture.replace(
        "        with:\n          fetch-depth: 0",
        "        env:\n          GH_TOKEN: ${{ secrets.SECRET_RELAY_ADMIN_TOKEN }}\n        with:\n          fetch-depth: 0",
      ),
      violation: "migration-job",
    },
  ])("rejects $name", ({ source, violation }) => {
    expect(source).not.toBe(canonicalMigrationFixture)
    expect(exactMigrationWorkflowViolations(source)).toContain(violation)
  })
})
