...
| --- | --- | --- |
| Edge chart payload number casts | Eight `float(object)` sites in `edge/chart_runtime.py` repeated the same type ignore and accidentally admitted booleans. One private `_parse_payload_number` now owns the wire boundary and fails with the field name. | `backend/tests/test_edge_chart_runtime.py` covers integer, float, numeric string, null, boolean, and invalid text. |
...
| --- | --- | --- |
| Skyfield type edge | Skyfield's `Time`, ephemeris vectors, and discrete-event arrays are untyped upstream. `Any` is confined to astronomy adapters in `calculations.py` and `strength/sunrise.py`; Pydantic chart models remain the typed output boundary. | `test_positions_apparent.py`, `test_node_speed.py`, `test_lagna_obliquity.py`, and `test_shadbala_components.py` exercise the adapter outputs. |
