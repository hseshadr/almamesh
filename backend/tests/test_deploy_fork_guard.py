...
deploy job then checks out ``workflow_run.head_sha`` and builds it with
``CLOUDFLARE_API_TOKEN`` and the production ``BUNDLE_PRIVATE_KEY_B64`` in scope.
That is arbitrary code execution holding the one signing key installed clients pin.
